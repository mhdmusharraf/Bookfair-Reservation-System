...
const HALLS = {
  N: { x: 60, y: 30, w: 230, h: 110, r: 12, capacity: 8, label: "HALL N" }, // bigger
...
/** size label mapping -> change to 'S' if you want uppercase */
...
function Entrance({ e }) {
  const cx = e.x + e.w / 2;
  const cy = e.y + e.h / 2;
  const arrowX = e.x + e.w - 18; // right side padding
  return (
    <g>
      {/* bar */}
      <rect
      ...
      {/* arrow AFTER the label (to the right) */}
      <text
        x={arrowX}
        y={cy}
        textAnchor="middle"
...
function EntranceSmallLeft({ e }) {
  const cx = e.x + e.w / 2;
  const cy = e.y + e.h / 2;
  const arrowX = e.x + e.w - 16; // near the right edge inside the rotated bar
  return (
...
export default function StallSvgMap({ stalls, selectedIds, onToggle }) {
  ...
  /** Size-based labels: s-1, M-1, L-1 as global counters by size */
    ...
    <div className="w-full">
      ...
      >
        ...
        {stalls.slice(0, 150).map((stall, i) => {
          ...
          const codeTop = sizeLabels[i]; // s-#, M-#, L-#

          return (
            <g
              key={stall.id}
              onClick={() => !isBooked && onToggle(stall)}
            ...
            >
              ...
              <title>{`${codeTop} — ${stall.size} — ${
                isBooked ? "Booked" : isSelected ? "In progress" : "Available"
              }`}</title>
            </g>
          );
        })}
